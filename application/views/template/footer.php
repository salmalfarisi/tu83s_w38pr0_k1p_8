</body>
<footer id="footer">

<div class=" footer1">
      <br>
      <p class=" awesome outfoot"> GEOFF-MAX GUARANTEE</p>
      <br><br>
      <div class="container" >
    <div class="row">
        <div class="col-md-4 col-sm-4" style="border-right: dotted 1px #000; ">
          <center>
            <img class ="imgfooter" src="https://cdn.gmx.shoes/assets/image/i-guarantee-quality-boys.svg?v=18.050">
            <p style="text-align: center;">Jaminan barang memiliki kualitas yang <br>terbaik</p></center>
        </div>
        <div class="col-md-4 col-sm-4">
          <center>
            <img class ="imgfooter" src="https://cdn.gmx.shoes/assets/image/i-guarantee-easey-boys.svg?v=18.050">
            <p style="text-align: center;">Kemudahan untuk menukarkan barang yang tidak sesuai dengan pesanan</p></center>
        </div>

        <div class="col-md-4 col-sm-4" style="border-left: dotted 1px #000;">
          <center>
            <img class ="imgfooter" src="https://cdn.gmx.shoes/assets/image/i-guarantee-money-boys.svg?v=18.050">
            <p style="text-align: center;">Jika ada suatu masalah kami jamin Uang anda kembali</p></center>
        </div></div>
  </div>
  <br><p class=" awesome outfoot2"> PAYMENT SUPPORT</p><br>
 <center><img src="<?php echo base_url('assets/contoh home/pic/bank.jpg')?>"></center> 
  
</div>
<div class="footer2">
  <br><br>
  <div class="footer-top">
    <div class="container">
      <div class="row">
        <div class="col-lg-2 col-md-4 footer-info">
          <h6 style="color:#FF6600 "><b>FLAGSHIP STORE</b></h6>
          <p style="color: white; font-size: 12px">Jl. Trunojoyo No.15 Bandung <br>
              Tlp. (022) 87301291</p>

          <h6 style="color:#FF6600 "><b>BANTUAN</b></h6>
          <p style="color: #36B44A"><img style="width: 31px; height: 31px" src="<?php echo base_url('assets/contoh home/pic/i-line.svg')?>"> @geoff_max</p>
</div>
<div class="col-lg-2 col-md-4 footer-links ">
  <h6 style="color:#FF6600"><b> LINK</b> </h6>
  <a href="#" style="color: white; font-size: 13px">Lokasi Store</a><br>
  <a href="#" style="color: white; font-size: 13px">Konfirmasi Pembayaran</a><br>
  <a href="#" style="color: white; font-size: 13px">About Us</a><br>
  <a href="#" style="color: white; font-size: 13px">Cara Beli</a><br>

  </div>
  <div class="col-lg-8 col-md-4 footer-contact text-right"><a href=""> <p style="color:#FF6600 "><b>FOLLOW US:</b></p>
    <a href=""><img style="width: 31px; height: 31px" src="<?php echo base_url('assets/contoh home/pic/fb.svg')?>"></a>
    <a href=""><img style="width: 31px; height: 31px" src="<?php echo base_url('assets/contoh home/pic/twitter.svg')?>"></a>
    <a href=""><img style="width: 31px; height: 31px" src="<?php echo base_url('assets/contoh home/pic/ig.svg')?>"></a>
    <br><br><br>
    <p style="color: white; font-size: 12px">Geoff Max © 2017 . Allright Reserved</p>
</div>
</div>
</div>
</div>
</div>
</footer>
</html>